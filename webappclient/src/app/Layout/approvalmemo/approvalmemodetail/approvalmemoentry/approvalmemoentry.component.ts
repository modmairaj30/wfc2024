...
@Component({
  selector: 'app-approvalmemoentry',
  templateUrl: './approvalmemoentry.component.html',
  styleUrls: ['./approvalmemoentry.component.scss']
})
export class ApprovalmemoentryComponent implements OnInit {
      ...
      data => {
        ...
        if (this.editmod == true) {
         this.editmode();
          ...
          this.invoice.noOfPieces=0;
          this.invoice.weight=0;
          this.invoice.cgst=0;
          this.invoice.sgst=0;
          this.invoice.igst=0;
          this.invoice.cgstAmt=0;
          this.invoice.sgstAmt=0;
          this.invoice.igstAmt=0;
          this.invoice.rptNetamt=0;
          this.invoice.netAmt=0;
          this.invoice.decVal=0;
          this.invoice.decamt=0;
          this.invoice.odCharges=0;
          this.invoice.fuelAmt=0;
          debugger;
          let sessionLocation=   window.sessionStorage.getItem("location");
          this.invoice.locationId = this.location_idList.find(x => x.id == sessionLocation);
  ...
  // this.lookUpFormBean11.query = " SELECT id,batch_no,cum_qty	FROM	tbl_stock_info where cum_qty>0 and  last_row='TRUE' and location_id=108";
    this.loading = true;
  ...
  terrifCalculation(){
   ...
   let  ct = new Credittarriffmaster();
    ...
    ct.locationId= this.invoice.consigneeDest.id;
  ...
  filterLocationId5(event) {
    let filtered: any[] = [];
    let query = event.query;
    for (let i = 0; i < this.teriff_locationList.length; i++) {
      let item = this.teriff_locationList[i];
      if (item.location_name.toLowerCase().indexOf(query.toLowerCase()) == 0) {
        filtered.push(item);
      }
    }
    this.filter_location_id5 = filtered;
  }

...
export class SalesinvoiceentryComponent implements OnInit {
  ...
  ngOnInit(): void {
    /////list
    this.salesinvoice.userId=window.sessionStorage.getItem("userName");
   // let location= window.sessionStorage.getItem("location");
    ...
    this.lookUpFormBean2.query ="select id, product_id, location_id, supp_name, qty, cum_qty, fld_sunit, qtytab, product_code, prod_name, fld_disc, isbn_code, product_group, rate, product_rate, fld_srate, discount from view_batch_cumqty_99 ";
    ...
    this.userService.getMultipleDropdwonValues(this.listLookUpFormBean).subscribe(
      data => {
       ...
       // this.salesinvoice.locationId = this.localList6.find(x => x.id == 15);

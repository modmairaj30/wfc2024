...
export class PaymentsHistoryComponent implements OnInit {
  ...
  @Output()
  editPaymentEntry = new EventEmitter();
public serverSidePagination = new ServerSidePagination();
  ...
  ngOnChanges(): void {
   ...
   ngOnInit(): void {
    ...
    this.loading = true;
    this.transactionService.getPayments(this.serverSidePagination).subscribe(
      data => {
        this.loading = false;
  ...
  editProduct(product: any) {
    this.editPaymentEntry.emit(product);
  }
  ...
  find(): void {
    this.loading = true;
    debugger;
    this.transactionService.findPayments(this.searchCriteria).subscribe(
      data => {
        this.loading = false;
        this.content = JSON.parse(JSON.stringify(data.data));
      },

...
@Component({
  selector: 'app-stockverificationhistoryreports',
  templateUrl: './stockverificationhistoryreports.component.html',
  styleUrls: ['./stockverification.scss'],
  providers: [MessageService]
})
export class StockverificationHistoryreportsComponent  implements OnInit {
  ...
  ngOnInit(): void {
    this.reportInput.fromDate= new Date();
    this.reportInput.toDate= new Date();
   /// let location= window.sessionStorage.getItem("location");
    //this.reportInput.locationId = this.locationList1.find(x => x.id == 1);
    this.reportInput.locationId = this.locationList1.find(x => x.id == 11);
...
//  copy.locationId=copy.locationId.id;
  this.loading=true;
  this.reportInput.locationId=this.reportInput.locationId.id;
  this.reportService.reportExportToExcel(this.reportInput).subscribe((blob: Blob): void => {
    this.loading=false;
    debugger;

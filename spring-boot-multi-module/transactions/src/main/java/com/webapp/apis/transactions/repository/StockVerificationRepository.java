...
@Repository
public interface StockVerificationRepository extends JpaRepository<StockVerification, Integer>, PagingAndSortingRepository<StockVerification, Integer>,JpaSpecificationExecutor<StockVerification> {
	public List<StockVerification> findAllByOrderByIdAsc();
	@Query("SELECT location FROM StockVerification location WHERE location.locationId IN (:locationId)")
	Page<StockVerification> findByLocationAll(@Param("locationId")List<Integer> locationId,Pageable pageable);
	@Query(value = "SELECT (max(id)+1) FROM StockVerification")
